...
import { DisasterPortal, getUserPortals, getUserVolunteerActivities, Volunteer } from "@/lib/db";
import { AlertTriangle, Calendar, Clock, Eye, Package, Plus, User } from "lucide-react";
      ...
      <main className="flex-1 container mx-auto px-4 py-8">
          ...
          <Card>
            <CardContent className="pt-6">
              <div className="flex flex-col items-center text-center">
                ...
                <p className="text-4xl font-bold text-primary mb-2">{userPortals.length}</p>
                <p className="text-sm text-gray-500">Relief portals created by you</p>
              </div>
            </CardContent>
          </Card>
          ...
          <Card>
            <CardContent className="pt-6">
              <div className="flex flex-col items-center text-center">
                ...
                <h3 className="text-xl font-semibold mb-1">Volunteer Activities</h3>
                <p className="text-4xl font-bold text-saffron mb-2">{volunteerActivities.length}</p>
                <p className="text-sm text-gray-500">Disaster relief efforts you've joined</p>
              </div>
            </CardContent>
          </Card>
            ...
            <CardContent className="pt-6">
              <div className="flex flex-col items-center text-center">
                ...
                <p className="text-4xl font-bold text-gray-600 mb-2">
                  {userPortals.length + volunteerActivities.length > 0 ? "Active" : "Inactive"}
                </p>
                <p className="text-sm text-gray-500">Your current contribution status</p>
              </div>
            </CardContent>
          </Card>
        </div>
        ...
        <Tabs defaultValue="portals" className="space-y-6">
            ...
            {loading ? (
                  ...
                  <Card key={portal.id} className="overflow-hidden">
                    <div className="flex flex-col md:flex-row">
                      <div className="md:w-1/4 bg-gray-50 p-4 flex flex-col justify-center items-center md:items-start border-b md:border-b-0 md:border-r border-gray-200">
                        <Badge className={`mb-2 ${portal.status === 'active' ? 'bg-green-500' : portal.status === 'resolved' ? 'bg-blue-500' : 'bg-gray-500'}`}>
                          {portal.status?.charAt(0).toUpperCase() + portal.status?.slice(1)}
                        </Badge>
                        <h3 className="text-lg font-semibold mb-1">{portal.title}</h3>
                        <p className="text-sm text-gray-500 mb-2">{portal.location}</p>
                        <div className="flex items-center text-sm text-gray-500">
                          <Calendar size={14} className="mr-1" />
                          {portal.createdAt ? new Date(portal.createdAt.toDate()).toLocaleDateString() : "No date"}
                        </div>
                      </div>

                      <div className="md:w-3/4 p-4">
                        <p className="text-gray-600 mb-4 line-clamp-2">{portal.description}</p>
                        <div className="flex flex-wrap gap-2 mb-4">
                          <Badge variant="outline" className="bg-gray-50">
                            {portal.disasterType?.charAt(0).toUpperCase() + portal.disasterType?.slice(1)}
                          </Badge>
                          <Badge variant="outline" className={`${portal.urgency === 'high' ? 'bg-red-50 text-red-700' : portal.urgency === 'medium' ? 'bg-yellow-50 text-yellow-700' : 'bg-green-50 text-green-700'}`}>
              ...
              <Card>
                <CardContent className="text-center py-12">
                  <p className="text-gray-500 mb-4">You haven't created any relief portals yet.</p>
                  <Button className="gap-2" asChild>
                    <Link href="/create-portal">
                      <Plus size={16} />
          ...
          <TabsContent value="volunteer">
            <div className="mb-4 flex justify-between items-center">
              <h2 className="text-xl font-semibold">Your Volunteer Activities</h2>
            ...
            {loading ? (
              <div className="text-center py-12">
                <div className="inline-block h-8 w-8 animate-spin rounded-full border-4 border-solid border-primary border-r-transparent align-[-0.125em] motion-reduce:animate-[spin_1.5s_linear_infinite]"></div>
                <p className="mt-4 text-gray-600">Loading your volunteer activities...</p>
              </div>
              ...
              <div className="space-y-4">
                {volunteerActivities.map((activity) => (
                  <Card key={activity.id} className="overflow-hidden">
                    <div className="flex flex-col md:flex-row">
                      <div className="md:w-1/4 bg-gray-50 p-4 flex flex-col justify-center items-center md:items-start border-b md:border-b-0 md:border-r border-gray-200">
                        <Badge className={`mb-2 ${activity.status === 'active' ? 'bg-green-500' : 'bg-gray-500'}`}>
                          {activity.status?.charAt(0).toUpperCase() + activity.status?.slice(1)}
                        </Badge>
                        <h3 className="text-lg font-semibold mb-1">{activity.name}</h3>
                        <div className="flex items-center text-sm text-gray-500 mb-2">
                          <Calendar size={14} className="mr-1" />
                          {activity.registeredAt ? new Date(activity.registeredAt.toDate()).toLocaleDateString() : "No date"}
                        </div>
                      </div>
                      ...
                      <div className="md:w-3/4 p-4">
                        ...
                        <div className="mb-4">
                          ...
                          <p className="text-gray-600">{activity.availability}</p>
                        </div>
              ...
              <Card>
                <CardContent className="text-center py-12">
                  <p className="text-gray-500 mb-4">You haven't volunteered for any relief efforts yet.</p>
                  <Button className="gap-2" variant="outline" asChild>
                    <Link href="/volunteer">
                      <Plus size={16} />
          ...
          <TabsContent value="contributions">
            ...
            <Card>
              <CardContent className="text-center py-12">
                <p className="text-gray-500 mb-4">You haven't made any resource contributions yet.</p>
                <Button className="gap-2" variant="outline" asChild>
                  <Link href="/resources">
                    <Plus size={16} />
